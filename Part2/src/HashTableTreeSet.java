...
public class HashTableTreeSet<K extends Comparable, V> implements KWHashMap<K, V> {
    ...
    private static class Entry<K extends Comparable,V> implements Comparable<Entry>{
        /** The key */
        private K key;
     ...
     */
    public HashTableTreeSet(int capacity){
        if (capacity > 0){
            this.capacity = capacity;
            table = new TreeSet[capacity];
        }
        else
            table = new TreeSet[this.capacity];
    }
    ...
    public String toString() {
        ...
        for (int i = 0; i < table.length; i++) {
            if (table[i] == null) {
                ...
                sb.append("\n");
                sb.append(String.format("|%7s| |%7s|", i, table[i].first().key));
                boolean flag=true;
